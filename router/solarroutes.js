...
const getSolarTelecaling = async (req, res) => {
  ...
  res.render("solar/telecaling_data", {
    prospects: TelecalingData,
    user: user,
  });
};
...
const getSolarShowEditTelecaling = async (req, res) => {
  const { id } = req.params;
  ...
  const telecaling = await SolarTelecalingData.findById(id);
  ...
  res.render("solar/editScreens/editScreen", {
    data: newObj,
    user,
  });
};

const getSolarShowEditStock = async (req, res) => {
  const { id } = req.params;
  ...
  const stockInwards = await SolarStockInwards.findById(id);
  console.log(stockInwards);

  res.render("solar/editScreens/stockInwards", { data: stockInwards, user });
};

const getSolarShowEditExpens = async (req, res) => {
  const { id } = req.params;
  let user = null;
  if (req.user) {
    user = req.user;
  }
  const expense = await SolarExpenses.findById(id);
  ...
  res.render("solar/editScreens/expenses", {
    data: newObj,
    user,
  });
};
...
const postSolarEditExpenses = async (req, res) => {
  const {
    id,
    executiveName,
    amount,
  ...
  const exp = await SolarExpenses.findById(id);
  ...
  res.redirect(`/solarShowEditExpens/${id}`);
};
...
const postSolarStockOutward = async (req, res) => {
  ...
  const stocks = await SolarStockInwards.findOne({
    productName: prodName.toUpperCase(),
  });
  if (stocks !== null) {
    ...
    await SolarStockInwards.findOneAndUpdate(
      { productName: prodName.toUpperCase() },
...
const postSolarEditTelecaling = async (req, res) => {
  const {
    id,
    custName,
    phoneNumber,
  ...
  try {
    if (status === "Follow Up" && !followUpDate) {
      ...
      res.redirect(`/solarShowEditTelecaling/${id}`);
      return;
    }
    const data = await SolarTelecalingData.findById(id);
...
const postSolarEditStockInwards = async (req, res) => {
  ...
  try {
    ...
    if (numberOfProducts < stock.numberOfProductsDifference) {
      ...
      res.redirect(`/solarShowEditStock/${id}`);
      return;
    }
